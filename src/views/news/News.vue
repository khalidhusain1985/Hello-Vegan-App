  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      ...
      <div class="row feed-body">
        <div class="col">
          <div v-for="v in news" class="card data-card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
            <RouterLink :to="'/news/detail/'+v?.slug">
              ...
              <div v-if="v?.media?.length" class="card-body p-0 mb-3 rounded-3 overflow-hidden">
                <div v-for="media in v?.media" >
                  <div :class="{'img-container':media?.ref_code=='news_image'}">
                    <img v-if="media?.ref_code=='news_image'" :src="media?.is_local?env.BASEPOINT+ media?.path:media?.path" class="img-fluid card-img mb-2">
                  </div>
                </div>
              </div>
            </RouterLink>
            <div class="card-body d-flex p-0">
              ...
              <SharerCard :title="v.title" :url="pitLib.util.url('news/'+v.slug)"></SharerCard>
            </div>
            <Comment class="mt-2" target="news" :target_id="v._id"> </Comment>
          </div>

          <div v-if="newsResponse?.data?.pages>page" class="row mt-2">
            <div class="col text-center">
              <button @click="page++;newsRefetch()" v-if="!loadingBlogsResponse" class="btn btn-primary btn-lg text-white rounded shadow-lg">
                Load More
...
const page=ref(1)
const newsResponse=ref<PaginatedApiResponse>()
const loadingBlogsResponse=ref<boolean>(true)
async function newsRefetch() {
  loadingBlogsResponse.value=true
  await axios.post<PaginatedApiResponse>(endpoints.news.index, {
    status:"active"
  }, {
    ...
    newsResponse.value=d.data
  })
}
watch(newsResponse,(vr:any)=>{
  if (vr.data?.data){
    // eslint-disable-next-line no-unsafe-optional-chaining
    news.value.push(...vr?.data?.data)
  }
})

async function likeToggle(v: any) {
  const item = news.value.find((item: any) => item._id === v._id);

  axios.post<PaginatedApiResponse|any>(endpoints.like.toggle, {
    target: "blog", target_id: v._id
  }, {
    headers: pitLib.util.headers(),
    params: {page: page.value}
  }).then(d => {
    ...
    if (d.status) {
      ...
      pitLib.notify("News "+((d?.data?.deletedCount)?"Unliked":"Liked")+" by "+pitLib.auth.get()?.name,pitLib.util.trimmer(v.title),v.created_by[0]._id);
    }
  })
}
