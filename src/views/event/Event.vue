        ...
        <div class="col">
          <div class="card w-100 shadow-xss rounded-xxl border-0 ps-4 pt-4 pe-4 pb-3 mb-3">
            <div class="card-body p-0">
              <RouterLink to="/event/create/new" class=" font-xssss fw-600 text-grey-500 card-body p-0 d-flex align-items-center"><i class="btn-round-sm font-xs text-primary fa-solid fa-plus me-2 bg-greylight"></i>
                Add New Event
              </RouterLink>
            </div>

          </div>
          <div v-for="v in events" class="card data-card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
            <RouterLink :to="'/event/'+v?.slug">
              ...
              <div v-if="v?.media?.length" class="card-body p-0 mb-3 rounded-3 overflow-hidden">
                <div v-for="media in v?.media" >
                  <div :class="{'img-container':media?.ref_code=='event_image'}">
                    <img v-if="media?.ref_code=='event_image'" :src="media?.is_local?env.BASEPOINT+ media?.path:media?.path" class="img-fluid card-img mb-2">
                  </div>
                </div>
              </div>
            </RouterLink>
            <div class="card-body d-flex p-0">
              ...
              <SharerCard :title="v.title" :url="pitLib.util.url('event/'+v.slug)"></SharerCard></div>
            <Comment class="mt-2" target="event" :target_id="v._id"> </Comment>
          </div>

          <div v-if="eventsResponse?.data?.pages>page" class="row mt-2">
            <div class="col text-center">
              <button @click="page++;eventsRefetch()" v-if="!loadingEventsResponse" class="btn btn-primary btn-lg text-white rounded shadow-lg">
                Load More
...
const page=ref(1)
const loadingEventsResponse=ref<boolean>(true)
const eventsResponse=ref<PaginatedApiResponse>()
...
function eventsRefetch() {
  loadingEventsResponse.value=true
  axios.post<PaginatedApiResponse>(endpoints.event.index, {status:"active"}, {
    headers: pitLib.util.headers(),
    params: {page: page.value}
  }).then(d=>{
    loadingEventsResponse.value=false
    eventsResponse.value=d.data
  })
}
...
async function likeToggle(v: any) {
  ...
  axios.post<PaginatedApiResponse|any>(endpoints.like.toggle, {
    ...
    if (d.status) {
      ...
      pitLib.notify("Event "+((d?.data?.deletedCount)?"Unliked":"Liked")+" by "+pitLib.auth.get()?.name,pitLib.util.trimmer(v.title),v.created_by[0]._id);
    }
  })
}
