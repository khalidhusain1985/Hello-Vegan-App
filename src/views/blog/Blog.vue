  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      ...
      <div class="row feed-body">
        <div class="col">
          <div v-for="v in blogs" class="card data-card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
            <RouterLink :to="'/blog/detail/'+v?.slug">
              ...
              <div v-if="v?.media?.length" class="card-body p-0 mb-3 rounded-3 overflow-hidden">
                <div v-for="media in v?.media">
                  <div :class="{'img-container':media?.ref_code=='blog_image'}">
                    <img v-if="media?.ref_code=='blog_image'" :src="media?.is_local?env.BASEPOINT+ media?.path:media?.path" class="img-fluid card-img mb-2">
                  </div>
                </div>
              </div>
            </RouterLink>
            <div class="card-body d-flex p-0">
              ...
              <SharerCard :title="v.title" :url="pitLib.util.url('blog/'+v.slug)"></SharerCard>
            </div>
...
async function blogsRefetch() {
  loadingBlogsResponse.value=true
   await axios.post<PaginatedApiResponse>(endpoints.blog.index, {
     status:"active"
   }, {
...
async function likeToggle(v: any) {
  ...
  axios.post<PaginatedApiResponse|any>(endpoints.like.toggle, {
    ...
    if (d.status) {
      ...
      pitLib.notify("Blog "+((d?.data?.deletedCount)?"Unliked":"Liked")+" by "+pitLib.auth.get()?.name,pitLib.util.trimmer(v.title),v.created_by[0]._id);
    }
  })
}
